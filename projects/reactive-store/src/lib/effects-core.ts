...
export type SideEffects = { [key: string]: SideEffectFunc<any> };
...
export function addSideEffect<T>(actionType: string, sideEffect: SideEffectFunc<T>) {
    sideEffects.set(actionType, [...(sideEffects.get(actionType) || []), sideEffect]);
}

export function removeSideEffect<T>(actionType: string, sideEffect: SideEffectFunc<T>) {
